        ...
        <main class="flex-1 overflow-x-hidden overflow-y-auto p-6 ml-64 space-y-4">
            ...
            <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="name" class="border w-full p-2" value="{{ $product->name }}">
                </div>

                <div>
                    <label class="block">Harga</label>
                    <input type="number" name="price" class="border w-full p-2" value="{{ $product->price }}">
                </div>

                <div>
                    <label class="block">Stok</label>
                    <input type="number" name="stock" class="border w-full p-2" value="{{ $product->stock }}">
                </div>
