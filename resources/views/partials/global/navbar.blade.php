<nav class="fixed top-0 left-0 w-full mx-auto flex justify-between items-center py-5 px-6 md:px-20 bg-[#F0F6FC] z-50">

    <!-- Logo -->
    <a href="{{ route('welcome') }}" class=""><img src="{{ asset('images/toko-nisa-logo-text-horizontal.svg') }}" alt="Logo Toko Nisa" class="h-6"></a>
    ...
    <div class="flex items-center gap-4">
        ...
        <div id="mobile-menu" class="hidden fixed top-16 left-0 w-full">
            <div class="flex flex-col items-center text-base bg-blue-100/90 backdrop-blur-lg z-50 gap-1 space-y-4 p-6 text-gray-800 md:hidden transition-all duration-300">
                ...
                @auth
                    <a href="{{ route('profile') }}" class="hover:underline text-base">Profil</a>
                @else
                    <a href="{{ route('login') }}" class="hover:underline text-base">Masuk</a>
                    <a href="{{ route('signup') }}" class="hover:underline text-base">Daftar</a>
                @endauth
            </div>
        </div>
